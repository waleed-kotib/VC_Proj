...
namespace eval ::tinyhttpd:: {
    ...
    set langfile [file join [file dirname [info script]] tinyhttpdmsg.tcl]
...
<!-- HTML generated by tinyhttpd -->}
    ...
    set html(tabletop,plain) {
	<table class="center" border=0 cellpadding="2">
	    <tr>
    ...
    set html(line,plain) {
    <tr>
	<td align="left"><a href="%s"><a href="%s"><img width="16" height="16" src="%s" alt=""/> %s </a></td>
	<td align="right"> %s </td>
    ...
    # Using CSS.
    set html(head,css) {<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">
	<html xmlns='http://www.w3.org/1999/xhtml' xml:lang='en'>
	<head>
	    <title>@dirlisting</title>
	    <meta http-equiv='Content-Type' content='text/html; charset=utf-8'/>
	    <link href="/httpd/std.css" type='text/css' rel='stylesheet'/>
	</head>
	<body>
	<!-- HTML generated by tinyhttpd -->
	<h2>@contains</h2>
    ...
    set html(updir,css) {
	<table class="center" width="600" border="0" cellspacing="1" cellpadding="4">
	    <tr>
		<td align="left">
		<a href="%s"><img src="%s" alt=""/>
		    @Parent (%s)
    ...
    set html(line,css) {
	<tr class="%s">
	    <td align="left"><a href="%s"><img width="16" height="16" src="%s" alt=""/> %s </a></td>
	    <td align="right"> %s </td>
	    <td align="left"> %s </td>
	    <td align="left"> %s </td>
	</tr>
    ...
    # Pyjamas colors.
    set html(class,css,0) white
    set html(class,css,1) blue
...
proc ::tinyhttpd::start {args} {
    ...
    if {[file pathtype $options(-rootdirectory)] ne "absolute"} {
	return -code error "the path must be an absolute path"
    }

    set servopts ""
    if {$options(-myaddr) ne ""} {
    ...
    set priv(rpath,imgs,folder) $priv(rpath,imgs)/folder.png
    set priv(rpath,imgs,file)   $priv(rpath,imgs)/file.png
    set priv(rpath,imgs,up)     $priv(rpath,imgs)/up.png
    
    set priv(apath,imgs,folder) [file join $priv(apath,imgs) folder.png]
    set priv(apath,imgs,file)   [file join $priv(apath,imgs) file.png]
    set priv(apath,imgs,up)     [file join $priv(apath,imgs) up.png]
    
    if {![file exists $priv(apath,imgs,folder)]} {
	return -code error "Failed localizing folder.png"
    }
...
proc ::tinyhttpd::NewChannel {s ip port} {
    ...
    # Be sure to accept any eol and always write CRLF.
...
proc ::tinyhttpd::Event {token} {    
	...
	if {[regexp -nocase {^([^:]+):(.+)$} $line - key value]} {
	    lappend state(meta) $key [string trim $value]
	}
	if {[regexp -nocase {^content-type:(.+)$} $line - type]} {
	    set state(content-type) [string trim $type]
	} elseif {[regexp -nocase {^accept-language:(.+)$} $line - langs]} {
	    set langs [string trim $langs]
	    set idx [string first "," $langs]
	    if {$idx == -1} {
		set lang $langs
	    } else {
		set lang [string range $langs 0 [expr {$idx-1}]]		
	    }
	    set state(lang) [lindex [split $lang "-"] 0]
	} elseif {[regexp -nocase {^range:(.+)$} $line - range]} {
...
proc ::tinyhttpd::PutHtmlDirList {token abspath} {
    ...
    # Do this always since line end translations may screw up the byte counts.
    fconfigure $s -buffering full -translation binary
...
proc ::tinyhttpd::BuildHtmlForDir {token} {    
    ...
    set map [list @dirlisting [Mc $lang dirlisting]  \
      @contains [Mc $lang contains $dirname $num]]
    set htmlStuff [string map $map $html(head,$style)]
    if {$relpath ne ""} {
	set map [list @Parent [Mc $lang Parent]]
	append htmlStuff [format [string map $map $html(updir,$style)] $uprpath $imgup $upname]
    }
    set map [list @Name [Mc $lang Name] @Size [Mc $lang Size]   \
      @Type [Mc $lang Type] @Date [Mc $lang Date]]
    append htmlStuff [string map $map $html(tabletop,$style)]
	...
	if {[string equal $style "css"]} {
	    ...
	    append htmlStuff [format $html(line,plain) \
	      $link $img $name $size $type $dateAndTime]
	}
	incr i
    }
    
    # And the end.
    set map [list @powered [Mc $lang powered]]
    append htmlStuff [string map $map $html(dirbottom,$style)]
...
proc ::tinyhttpd::Mc {lang src args} {
    variable msg
    
    if {[info exists msg($lang,$src)]} {
	return [eval {format $msg($lang,$src)} $args]
    } else {
	return $src
    }
}

proc ::tinyhttpd::McSet {lang src {dst ""}} {
    variable msg

    set msg($lang,$src) $dst
}

proc ::tinyhttpd::McLoad {} {
    variable msg
    variable langfile
